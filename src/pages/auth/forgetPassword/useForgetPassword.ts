...
import { ForgetPasswordFrom, ForgetPasswordResponse } from "./interface";
import { useNavigate } from "react-router-dom";

const useForgetPassword = () => {
  const { onChange, form } = useForm<ForgetPasswordFrom>({
    email: "",
  });
  ...
  const handleRecoverAccount = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const { estado } = await loadApi<ForgetPasswordResponse>({
